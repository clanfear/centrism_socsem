...
```{r, echo=FALSE, warning=FALSE, message=FALSE, dpi=1500, fig.width =7, fig.height=5, out.width="700px"}
...
annotation_df <- tibble(x_curve_start = c(2,3), 
                        x_curve_end = c(2.35, 3.35), 
                        y_curve_start = c(.41, .36),
                        y_curve_end = c(.49, .43),
                        x_label = c(3, 4),
                        y_label = c(.49, .43),
...
ggplot(min_wage_party, aes(x=Position, y = Prop, fill = Color)) + geom_col() +
  ...
  geom_curve(aes(x = x_curve_start, y = y_curve_start, xend = x_curve_end, yend=y_curve_end), curvature = -0.45, angle = 90, arrow = arrow(angle = 30, length = unit(0.03, "npc"), ends = "first"), lwd = 0.6, data = annotation_df, inherit.aes=FALSE) + 
  geom_hline(aes(yintercept = 0)) + 
  geom_text(aes(x = x_label, y = y_label, label = "Median Preferred\nMinimum Wage"), family = "Quattrocento", data = annotation_df, inherit.aes=FALSE) +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, dpi=1500, fig.width =6, fig.height=6, out.width="520px", fig.align='center'}
...
typology_df <- tibble(
  x_loc = c(0.35, 0.65, 0.8),
  y_loc = c(0.475, 0.575, 0.4),
  label = c("Electorate\nCentrism", "Partisan\nCentrism", "Elite\nCentrism")
)
...
ggplot(typology_df, aes(x=x_loc, y=y_loc, label = label)) + 
  ...
  coord_fixed(xlim=c(0,1), ylim=c(0,1)) +
    geom_hline(aes(yintercept = .5), alpha = 0.3) +  
  geom_vline(aes(xintercept = .5), alpha = 0.3) + 
  geom_circle(aes(x0 = x_loc, y0 = y_loc, r = radius, fill = affiliation), color=NA, data = electorate_df, inherit.aes = F, alpha=0.3) +
  geom_circle(aes(x0 = x_loc, y0 = y_loc, r = radius, fill = party), color=NA, data = party_df, inherit.aes = F, alpha=0.6) +
    geom_text(nudge_x = 0.06, nudge_y = -0.045, family = "Quattrocento") + 
  geom_text(aes(label=party), family = "Quattrocento", data = party_df) + 
  geom_text(aes(label=affiliation), family = "Quattrocento", data = electorate_df) + 
  scale_fill_manual(values = c("Democratic\nElectorate" = "blue", "Democratic\nParty" = "blue", "Republican\nElectorate" = "red", "Republican\nParty" ="red")) +
  theme_minimal(base_family = "Quattrocento") +
  xlab("Economic") + ylab("Social") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none", axis.text = element_blank())
